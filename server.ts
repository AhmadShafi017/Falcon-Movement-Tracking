      ...
      if (action === "all-latest") {
        ...
        if (!targetDateStr) {
          const latestDateRes = await runQuery(
            `SELECT TO_CHAR(MAX(APPLY_DATE_TIME), 'YYYY-MM-DD') AS LATEST_DATE FROM USER_LOCATION WHERE EMP_ID = :id`,
            [empId]
          );
          const rows: any = latestDateRes.rows;
          targetDateStr = (rows && rows.length > 0 && rows[0].LATEST_DATE) ? rows[0].LATEST_DATE : null;
        }
    ...
    try {
      ...
      const sqlReport = `
        ...
        ORDER BY APPLY_DATE ASC, 
          CASE 
            WHEN REGEXP_LIKE(TIME_STR, '^(0?[0-9]|1[0-2]):[0-9]{2} (AM|PM)$', 'i') 
            THEN TO_DATE(APPLY_DATE || ' ' || TIME_STR, 'YYYY-MM-DD HH:MI AM')
            ELSE TO_DATE(APPLY_DATE || ' ' || TIME_STR, 'YYYY-MM-DD HH24:MI:SS')
          END ASC
      `;
