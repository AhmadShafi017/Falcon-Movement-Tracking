...
import { cachedFetch, invalidateCache } from './hooks/useApiCache';
import { useScheduledCache } from './hooks/useScheduledCache';
...
export default function App() {
  ...
  const setCurrentPage = useCallback(async (page: 'MOVEMENT' | 'LOCATION' | 'REPORT') => {
    try {
  ...
  const handleManualUnlockSubmit = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      }).catch(err => console.warn("popstate sync failed:", err));
    };
    window.addEventListener('popstate', handlePopState);
    return () => window.removeEventListener('popstate', handlePopState);
  }, []);

  // Initial mount: clean URL, detect page from path, fetch initial data
  useEffect(() => {
  ...
  // Scheduled cache prewarming at :25 and :55 past each hour
  useScheduledCache();

  // Fetch employees once
  useEffect(() => {
    cachedFetch<any[]>('/api/employees').then(data => {
      setEmployees(Array.from(new Map(data.map((item: any) => [item.EMP_ID, item])).values()) as Employee[]);
    }).catch(console.error);
  }, []);
  ...
  const setSelectedEmpIdCallback = useCallback((id: string) => setSelectedEmpId(id), []);
  const setShowHospitalsCallback = useCallback((v: boolean) => setShowHospitals(v), []);
  const setShowCustomersCallback = useCallback((v: boolean) => setShowCustomers(v), []);
  const setPoiLoadingCallback = useCallback((v: boolean | ((prev: boolean) => boolean)) => {
  ...
  const setSearchQueryCallback = useCallback((q: string) => setSearchQuery(q), []);
  const setSelDivCallback = useCallback((v: string) => setSelDiv(v), []);
  const setSelNHCallback = useCallback((v: string) => setSelNH(v), []);
  const setSelZoneCallback = useCallback((v: string) => setSelZone(v), []);
  const setSelRegionCallback = useCallback((v: string) => setSelRegion(v), []);
  const setSelAreaCallback = useCallback((v: string) => setSelArea(v), []);
  const setSelTerrCallback = useCallback((v: string) => setSelTerr(v), []);
  const setMapStyleCallback = useCallback((v: 'hybrid' | 'roadmap') => setMapStyle(v), []);
  const setStatusFilterCallback = useCallback((v: 'all' | 'active' | 'hibernate' | 'leave' | 'authorized_leave' | 'unauthorized_leave') => setStatusFilter(v), []);
  const setRoleFilterCallback = useCallback((v: string) => setRoleFilter(v), []);
  const setActivePointCallback = useCallback((p: MovementPoint | null) => setActivePoint(p), []);

  const sharedProps = useMemo(() => ({
    loading, employees, searchQuery, setSearchQuery: setSearchQueryCallback,
    selDiv, setSelDiv: setSelDivCallback,
    selNH, setSelNH: setSelNHCallback,
    selZone, setSelZone: setSelZoneCallback,
    selRegion, setSelRegion: setSelRegionCallback,
    selArea, setSelArea: setSelAreaCallback,
    selTerr, setSelTerr: setSelTerrCallback,
    handleClearFilters: handleGlobalReset,
    selectedEmpId, setSelectedEmpId: setSelectedEmpIdCallback,
    location, activePoint, handlePointSelect: setActivePointCallback,
    addressCache, mapStyle, setMapStyle: setMapStyleCallback,
    ...
    loading, employees, searchQuery, setSearchQueryCallback,
    selDiv, setSelDivCallback, selNH, setSelNHCallback,
    selZone, setSelZoneCallback, selRegion, setSelRegionCallback,
    selArea, setSelAreaCallback, selTerr, setSelTerrCallback,
    handleGlobalReset, selectedEmpId, setSelectedEmpIdCallback,
    location, activePoint, setActivePointCallback,
    addressCache, mapStyle, setMapStyleCallback,
    totalDistance, filteredGlobalLocations, allLatestLocations,
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        {currentPage === 'REPORT' && (
          <ReportPage 
            ...
            setSelectedEmpId={setSelectedEmpIdCallback}
