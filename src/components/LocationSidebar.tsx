...
export const LocationSidebar: React.FC<LocationSidebarProps> = memo(({
  loading,
  employees,
  searchQuery,
  setSearchQuery,
  ...
  const ROLE_LEVEL_MAP: Record<string, string> = { 'RM': '4', 'AM': '5', 'MPO': '6' };
  ...
  // Counts for Signal Intelligence: filtered by hierarchy + roleFilter (not statusFilter, since status cards are the filter)
  ...
  return (
    <aside className="w-96 h-full border-r border-slate-100 flex flex-col bg-white shrink-0 z-10 shadow-sm overflow-hidden">
      ...
      {/* ── SECTION 1: Signal Intelligence (always visible, collapsible) ── */}
      <div className="p-6 bg-slate-50/50 border-b border-slate-100 space-y-4 border-l-4 border-l-emerald-500 shrink-0">
      ...
      {/* ── SECTION 2: Deployment Hierarchy filters (always visible, collapsible) ── */}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {loading ? (
          ...
          /* ── No employee selected: show fleet overview + filtered list ── */
          <div className="h-full flex flex-col items-center justify-center p-8 text-center text-slate-400 space-y-5">
            ...
            <div className="space-y-4 w-full">
              ...
              {(selRegion || selArea || selTerr) && filteredGlobalLocations.length > 0 && (
                <div className="px-4 space-y-2 text-left">
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2">
                    Trackable Assets (GPS Online)
                  </p>
              ...
              <p className="text-[9px] text-slate-400 font-bold px-6 leading-relaxed text-center uppercase tracking-tighter">
                {selTerr || selArea || selRegion
                  ? 'Select a specific asset from the list or map to view detailed telemetry'
                  : 'Drill down into regions or areas to sync personnel locations'}
              </p>
            </div>
          </div>
        ) : location ? (
          /* ── Employee selected: show detail view ── */
